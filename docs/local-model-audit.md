...
|------|---------------|----------|----------|----------|
| Embedding | `BAAI/bge-m3` | `/Users/xuegang/Desktop/My Project/bge-m3-local`，以及 HF 缓存 `/Users/xuegang/.cache/huggingface/hub/models--BAAI--bge-m3` | 权重存在，服务未运行 | 复用现有 FastAPI 服务，端口改为 `8002` |
| Vector DB | Milvus standalone | Docker 镜像 `milvusdb/milvus:v2.6.18`、`quay.io/coreos/etcd:v3.5.25`、`minio/minio`、`zilliz/attu:v2.6.3` | Attu 正在 `8000`，Milvus/etcd/minio 已退出 | 迁移或修复 compose，重新启动 Milvus |
...
- 服务未运行。
- `8000` 已被 Milvus Attu 占用。

建议启动方式：

```bash
cd "/Users/xuegang/Desktop/My Project/bge-m3-local"
BGE_PORT=8002 ./scripts/start.sh mps
curl http://127.0.0.1:8002/info
```
...
milvus-etcd        Exited
milvus-minio       Exited
milvus-attu        Up，127.0.0.1:8000->3000
```

当前发现的问题：

- Milvus API `127.0.0.1:19530` 不可连接。
- Milvus WebUI health `127.0.0.1:9091/healthz` 不可连接。
...
```http
POST http://127.0.0.1:8003/rerank
```
...
1. 修复并启动 Milvus：先保证 `19530` 可用。
2. 启动 bge-m3：使用 `8002`，验证 `/v1/embeddings`。
3. 下载并部署 reranker：使用 `8003`，验证 query-document score。
4. 下载 Qwen GGUF 并安装 llama.cpp：使用 `8004/v1`，验证 chat completion。
5. 启动本项目 backend：使用 `8080`，串联 embedding、Milvus、reranker、LLM。
6. 启动 frontend：使用 `3001`。
